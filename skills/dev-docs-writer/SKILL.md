...
3. **文章表現パス**: `references/ai-slop-checklist.md` を開き、**すべての段落**に対して
   §10のセルフチェック（現在7問）を通す。1問でも「YES」が残る段落があれば、その段落を
   書き直す。「YES」が残る段落がゼロになるまで抜けない。

   **証跡の残し方（AIエージェントが実行する場合の必須プロセス）**:  
   「全段落に通した」と宣言する前に、次のいずれかの形で実行の証跡を残す。
   宣言だけで済ませると、実際には確認していない段落が残りやすい（curse of knowledge）。

   - **slopフラグが立った場合**: 「段落X（先頭N文字を引用）→ §Yに引っかかる（該当語や
     問題箇所を具体的に指摘）→ 修正内容」の形で列挙する
   - **フラグゼロで通した場合**: 「以下N段落を確認、slopフラグは0件だった。段落1: [要点],
     段落2: [要点], ...」の形で確認した段落を列挙する

   ユーザーには「全段落を確認した」の一言ではなく、上記の証跡ごと報告する。証跡を書けない
   場合は、実際には全段落を確認していない可能性が高いので、確認からやり直す。
...
**編集の文章表現パス（Step 3.3）では、すべての段落を ai-slop-checklist.md §10 の質問に通す**
（現在7問。論理関係の曖昧化・横文字カタカナジャーゴンのチェックを含む）。1問でも「YES」が
残る段落があれば、その段落は slop なので書き直す。「だいたい潰した」で終わらないこと
（curse of knowledge により、書き手には自分の slop が最も見えにくい）。実行後は Step 3.3
に記載した「証跡の残し方」に従って、確認した段落と検出した slop を列挙して報告する。
